...
async function main() {
  ...
  for (let y = 130; y < height - 30; y += 2) {
    ...
    // Lower saturation threshold to catch neutrals, but still exclude cream background
    if (hsl.s > 0.05 && hsl.l < 0.97 && hsl.l > 0.03) {
  ...
  // Merge close peaks (within 3 buckets = 24px)
  const merged = [];
