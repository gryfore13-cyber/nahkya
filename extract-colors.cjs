...
async function main() {
  ...
  // Merge peaks that are within 3 buckets (~18px) - they belong to the same circle
  const merged = [];
  ...
  results.forEach((r, i) => {
    console.log(`  ${String(i + 1).padStart(2)}. y=${r.y.toString().padStart(4)} | n=${r.count.toString().padStart(3)} | hex=${r.hex}`);
  });

  // Group into families (gap > 55)
  const families = [];
  let fam = [];
  ...
  fs.writeFileSync(
    'D:/NAHKYA/extracted-colors.json',
