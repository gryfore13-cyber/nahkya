    ...
    <div className="min-h-screen bg-nahkya-ivory">
      ...
      <div className="bg-nahkya-charcoal py-24 px-5">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="font-display text-display-lg text-nahkya-ivory font-medium mb-4">By Nahkya</h1>
      ...
      {featured && (
        <div className="max-w-6xl mx-auto px-5 py-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div className="aspect-4/3 bg-nahkya-stone rounded-nahkya overflow-hidden">
              <img src={featured.image} alt={featured.title} className="w-full h-full object-cover" />
            </div>
            <div>
              <p className="font-mono text-mono-sm text-nahkya-gold uppercase tracking-widest-alt mb-3">{featured.category}</p>
              <h2 className="font-display text-display-sm text-nahkya-text font-medium mb-4">{featured.title}</h2>
              <p className="text-body-md text-nahkya-text-muted font-body mb-6">{featured.excerpt}</p>
              <div className="flex items-center gap-4">
                <span className="text-sm text-nahkya-text font-body">{featured.author}</span>
                <span className="font-mono text-mono-sm text-nahkya-text-muted uppercase">{featured.date}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {columnArticles.map((a) => (
            <div key={a.id} className="group">
              ...
              <p className="text-body-md text-nahkya-text-muted font-body line-clamp-2 mb-3">{a.excerpt}</p>
              <div className="flex items-center gap-3">
                <span className="text-sm text-nahkya-text font-body">{a.author}</span>
                <span className="font-mono text-mono-sm text-nahkya-text-muted">{a.date}</span>
              </div>
            </div>
          ))}
        </div>
