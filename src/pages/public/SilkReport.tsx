    ...
    <div className="min-h-screen bg-nahkya-bg">
      <div className="bg-nahkya-text py-24 px-5">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="font-display text-display-lg text-nahkya-bg font-medium mb-4">The Silk Report</h1>
          ...
          {CATEGORIES.map((c) => (
            <button key={c} onClick={() => setActive(c)}
              className={`px-4 py-2 font-mono text-mono-sm font-medium uppercase tracking-label rounded-nahkya transition-colors ${
                active === c ? 'bg-nahkya-accent text-nahkya-inverse' : 'bg-nahkya-surface border border-nahkya-border text-nahkya-text-secondary hover:border-nahkya-accent'
              }`}>{c}</button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filtered.map((a) => (
            <div key={a.id} className="group">
              ...
              <p className="text-body-md text-nahkya-text-secondary font-body line-clamp-2">{a.excerpt}</p>
            </div>
          ))}
        </div>
        {filtered.length === 0 && (
