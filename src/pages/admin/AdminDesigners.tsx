    ...
    <div className="p-8 lg:p-12">
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="relative flex-1 max-w-xs w-full">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2 bg-nahkya-surface border border-nahkya-border text-nahkya-text text-sm rounded-nahkya focus:outline-none focus:border-nahkya-highlight w-full font-body"
          />
        </div>
        <p className="font-mono text-mono-sm text-nahkya-text-secondary uppercase">{filtered.length} designers</p>
      </div>

      {/* Table */}
      <div className="bg-nahkya-surface border border-nahkya-border rounded-nahkya overflow-hidden">
        <div className="hidden lg:grid gap-3 px-5 py-3 border-b-2 border-nahkya-text" style={{ gridTemplateColumns: '60px 1fr 1fr 100px 100px 100px 100px 80px' }}>
          {['', 'Name', 'Email', 'Rate', 'Earned', 'Commissions', 'Status', ''].map((h, i) => (
            <span key={`${h}-${i}`} className="font-mono text-mono-sm font-medium uppercase text-nahkya-text-secondary">
              {h}
            </span>
          ))}
        </div>
        {isLoading && (
          <div className="px-5 py-12 text-center text-nahkya-text-secondary font-body">Loading designers...</div>
        )}
        {!isLoading && filtered.length === 0 && (
          <div className="px-5 py-12 text-center text-nahkya-text-secondary font-body">No designers found.</div>
        )}
        {filtered.map((designer, i) => {
          ...
          return (
            <div
              ...
              style={{ gridTemplateColumns: '60px 1fr 1fr 100px 100px 100px 100px 80px' }}
            >
              <div className="flex items-center justify-center">
                {designer.avatar ? (
                  <img src={designer.avatar} alt={designer.name} className="w-10 h-10 rounded-full object-cover" />
                ) : (
                  <div className="w-10 h-10 rounded-full bg-nahkya-surface-raised border border-nahkya-border flex items-center justify-center">
                    <User className="w-4 h-4 text-nahkya-text-secondary" strokeWidth={1.5} />
                  </div>
                )}
              </div>
              <div>
                <p className="text-sm text-nahkya-text font-body font-medium">{designer.name}</p>
                <p className="font-mono text-mono-sm text-nahkya-text-secondary uppercase">{designer.bio.slice(0, 60)}{designer.bio.length > 60 ? '...' : ''}</p>
              </div>
              <span className="text-sm text-nahkya-text font-body">{designer.email}</span>
              <span className="font-mono text-mono-sm text-nahkya-highlight uppercase">BND {designer.commissionRate}</span>
              <span className="font-mono text-mono-sm text-nahkya-text">BND {stats.totalEarned}</span>
              <span className="font-mono text-mono-sm text-nahkya-text-secondary">{stats.commissionCount}</span>
              <span className={cn('font-mono text-mono-sm uppercase ', designer.isActive ? 'text-nahkya-success' : 'text-nahkya-text-secondary')}>
                {designer.isActive ? 'Active' : 'Inactive'}
              </span>
              <div className="flex items-center gap-2 justify-end">
                <button onClick={() => openEdit(designer)} className="p-1.5 text-nahkya-text-secondary hover:text-nahkya-highlight transition-colors">
                  <Pencil className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => setConfirmDelete(designer.id)} className="p-1.5 text-nahkya-text-secondary hover:text-nahkya-error transition-colors">
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-modal flex items-center justify-center">
          ...
          <div className="relative bg-nahkya-bg border border-nahkya-border rounded-nahkya max-w-prose w-full max-h-90vh overflow-y-auto p-8">
            ...
            <div className="space-y-5">
              {[
                ...
                <div key={field.key}>
                  <label className="block font-mono text-mono-sm font-medium uppercase text-nahkya-text-secondary mb-2">
                    {field.label}
                  </label>
                  {field.type === 'textarea' ? (
                    <textarea
              ...
              <div>
                ...
                <button
                  ...
                  className={cn('flex items-center gap-4 rounded-nahkya border border-nahkya-border bg-nahkya-surface p-3 hover:border-nahkya-highlight transition-colors ', uploadingAvatar ? 'opacity-50' : '')}
                >
                  {uploadingAvatar ? (
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={cn('w-10 h-5 rounded-full cursor-pointer transition-colors ', form.isActive ? 'bg-nahkya-success' : 'bg-nahkya-border')}
                    onClick={() => setForm({ ...form, isActive: !form.isActive })}
                  >
                    <div className={cn('w-4 h-4 rounded-full bg-nahkya-inverse mt-0.5 transition-transform ', form.isActive ? 'translate-x-5' : 'translate-x-0.5')} />
                  </div>
            ...
            <div className="pt-6 border-t border-nahkya-border mt-6 flex gap-3">
              <button
                onClick={handleSave}
                className="px-6 py-2.5 bg-nahkya-highlight text-nahkya-text text-sm font-body font-medium rounded-nahkya hover:bg-nahkya-border transition-colors"
              >
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-modal flex items-center justify-center">
          <div className="absolute inset-0 bg-nahkya-text/50" onClick={() => setConfirmDelete(null)} />
          <div className="relative bg-nahkya-bg border border-nahkya-border rounded-nahkya max-w-auth w-full p-8">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="px-5 py-2 bg-nahkya-error text-nahkya-inverse text-body-sm font-body font-medium rounded-nahkya hover:opacity-90 transition-opacity"
              >
                Delete
              </button>
              <button
                onClick={() => setConfirmDelete(null)}
                className="px-5 py-2 border border-nahkya-border text-body-sm font-body text-nahkya-text rounded-nahkya hover:border-nahkya-text transition-colors"
              >
                Cancel
              </button>
            </div>
